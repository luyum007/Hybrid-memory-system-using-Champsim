...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    if (cache_type == IS_LLC)//venkat
    {
        // way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type,MSHR.entry[mshr_index].read_write);
    ...
    if (block[set][way].dirty)
    {
      ...
      if (lower_level)
      {
        ...
        else
        {
          ...
          writeback_packet.w = way; // added by venkat
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
  {
    ...
    { // writeback miss (or RFO miss for L1D)
      ...
      { // RFO miss
        ...
        { // this is a new miss
          ...
          else
          {
            ...
            WQ.entry[index].w = way; // added by venkat
            add_mshr(&WQ.entry[index]);
      ...
      else
      {
        ...
        if (cache_type == IS_LLC)//venkat
        {
          // way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type,WQ.entry[index].read_write);
...
void CACHE::handle_read()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      RQ.entry[index].w = way; // added by venkat
          ...
          if (lower_level)
          {
            if (cache_type == IS_LLC)
            {
              ...
              else
              {
                ...
                PQ.entry[index].w = way; // added by venkat
...
void CACHE::operate()
{
  // setting timer for 5 million clock cycles
    
    if(current_core_cycle[cpu] - arbitary_value > 5000000 && cache_type == IS_LLC)
    {
      // cout<<"resetting llc accesses"<<endl;
      arbitary_value = current_core_cycle[cpu];
      priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, Compare> pq;
      for(int i=0;i<NUM_SET;i++)
      {
        pq.push({llc_sets[cpu][i],i});
      }
      int count_sram = 0;
      // replace blocks from the set with the least accesses
      for(int i=0;i<NUM_SET/2;i++)
      {
        int set = pq.top().second;
        pq.pop();
        for(int j = 0;j<16;j++)
        {
          if(llc_blocks[cpu][16*i+j] == 0)
          {
            count_sram++;
            BLOCK& fill_block = block[i][j];//current block in the set 

            // handle the oldest entry
            PACKET& handle_pkt = PQ.entry[PQ.head];
            fill_cache(i, j, &handle_pkt);
            PQ.entry[PQ.head].pf_metadata = llc_prefetcher_cache_fill(PQ.entry[PQ.head].address<<LOG2_BLOCK_SIZE, i, j, (PQ.entry[PQ.head].type == PREFETCH) ? 1 : 0, block[i][j].address<<LOG2_BLOCK_SIZE, PQ.entry[PQ.head].pf_metadata);
            // block from prefetcher
          }
          else
          {
            break;
      ...
      // if(count_sram == 0)
      // {
      //   cout<<"no sram blocks"<<endl;
      // }
      // else{
      //   std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, Compare> emptyPq;
      //   pq.swap(emptyPq); // Swap with an empty priority queue
      //   for(int i=0;i<NUM_SET;i++)
      //   {
      //     pq.push({llc_sets[cpu][i],i});
      //   }
...
int CACHE::add_rq(PACKET *packet)
{
  ...
  // print way of the block

  // cout<< get_way(packet->address, get_set(packet->address))<<endl;
  // if(RQ.entry[index].acc_type == 1 && cache_type == IS_LLC && RQ.entry[index].w>=4) // and  
  // {
  //   // cout<<"10"<<end;
  //   // cout<< get_way(packet->address, get_set(packet->address))<<endl;
  //   if (RQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
  //     RQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + WRITE_LATENCY;
  //   else
  //     RQ.entry[index].event_cycle += WRITE_LATENCY;
  // }
  // else
  // {
    if (RQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
...
int CACHE::add_wq(PACKET *packet)
{
  ...
  // added by venkat 
  uint32_t hit_way = check_hit(&WQ.entry[index]);
  // if(cache_type == IS_LLC)
  // {
  //   // cout<<hit_way<<endl;
  //   if(hit_way>=4 && hit_way<16)
  //   {
  //     if (WQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
  //       WQ.entry[index].event_cycle = current_core_cycle[packet->cpu] + WRITE_LATENCY;
  //     else
  //       WQ.entry[index].event_cycle += WRITE_LATENCY;
  //   }
  //   else
  //   {
      if (WQ.entry[index].event_cycle < current_core_cycle[packet->cpu])
...
void CACHE::return_data(PACKET *packet)
{
  ...
  // added by me
  //  ADD LATENCY
  // if(cache_type == IS_LLC && MSHR.entry[mshr_index].read_write==0)// writes would be directed to sram
  // {
  //   if (MSHR.entry[mshr_index].event_cycle < current_core_cycle[packet->cpu])
  //     MSHR.entry[mshr_index].event_cycle = current_core_cycle[packet->cpu] + WRITE_LATENCY;
  //   else
  //     MSHR.entry[mshr_index].event_cycle += WRITE_LATENCY;
  // }
  // else
  // {
    if (MSHR.entry[mshr_index].event_cycle < current_core_cycle[packet->cpu])
