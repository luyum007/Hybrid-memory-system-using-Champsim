...
int CACHE::add_rq(PACKET *packet)
{
  ...
  if(RQ.entry[index].acc_type == 1 && cache_type == IS_LLC && hit_way>=4 && hit_way<16) // and  
  {
    // cout<<"10"<<end;
    // cout<< get_way(packet->address, get_set(packet->address))<<endl;
...
void CACHE::return_data(PACKET *packet)
{
  ...
  // added by me
  //  ADD LATENCY
  if(cache_type == IS_LLC && MSHR.entry[mshr_index].read_write==0)// writes would be directed to sram  reads to stt
  {
