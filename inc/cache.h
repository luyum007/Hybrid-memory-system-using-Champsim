...
#define L2C_WRITE_LATENCY 10 // 4/5 (L1I or L1D) + 10 + 10 = 24/25 cycles
...
#define LLC_WRITE_LATENCY 40 // 4/5 (L1I or L1D) + 10 + 20 + 40 = 74/75 cycles



// added by venkat
#define NO_BLOCKS 32768
#define NO_SETS 2048
// compare function for min priority queue
struct Compare {
  bool operator()(const std::pair<u_int64_t, uint64_t>& p1, const std::pair<uint64_t, uint64_t>& p2) const {
    // Compare first element of pairs
...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t sim_access[NUM_CPUS][NUM_TYPES],
             ...
             // added by venkat
             llc_blocks[NUM_CPUS][NO_BLOCKS],
             llc_sets[NUM_CPUS][NO_SETS],
             arbitary_value = 0;
    ...
    uint32_t get_set(uint64_t address),
             ...
             llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type,int read_write),//venkat
             lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
             lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type,int read_write);//venkat
};
